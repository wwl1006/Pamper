...
public class NoticeController {
    ...
    public Object sendNotice(@RequestHeader("token") String token, @RequestBody Notice notice) {
        return noticeService.sendNotice(token, notice);
    }

    @RequestMapping(path = "/myNotices", name = "获取我的通知列表接口", method = RequestMethod.GET)
    public Object getMyNotices(@RequestHeader("token") String token) {
    ...
    @RequestMapping(path = "/{id}", name = "获取通知详情接口", method = RequestMethod.GET)
    public Object getNoticeById(@RequestHeader("token") String token, @PathVariable Long id) {
        return noticeService.getNoticeById(token, id);
    }

    @RequestMapping(path = "/mySent", name = "获取我发送的通知列表接口", method = RequestMethod.GET)
    public Object getMySentNotices(@RequestHeader("token") String token) {
